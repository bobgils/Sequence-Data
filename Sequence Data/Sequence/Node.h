...
//     a new list that contains the same NodeItems as the list pointed to by
...
//    new list that contains the NodeItems from start_ptr up to but not including 
...
public:
    ...
    // CONSTRUCTOR
//    Node(const value_type& init_data = value_type(), Node<NodeItem>* init_link = NULL);
    Node() {};

    // Member functions to set the data and link fields:
    void set_data(const value_type& );
    void set_link(Node<NodeItem>*);

    // Constant member function to retrieve the current data:
    value_type data() const;

    // Two slightly different member functions to retreive the current link:
    const Node<NodeItem>* link() const;
    Node<NodeItem>* link();

private:
    value_type data_field;
    Node<NodeItem>* link_field;
};
