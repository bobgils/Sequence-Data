...
int test4()
{
    ...
    if (!correct(copy2, 2 * TESTSIZE, 2 * TESTSIZE - 1, items))
        return 0;
    ...
    original.advance();
    original.remove_current(); // Delete 2 from original, but not the copy.
    if (!correct(copy3, 2 * TESTSIZE - 1, TESTSIZE, items))
        return 0;
    ...
    original.advance();
    original.remove_current(); // Delete 2 from original, but not the copy.
    if (!correct(copy4, 2 * TESTSIZE - 1, 0, items))
        return 0;
    ...
    original.advance();
    original.remove_current(); // Delete 2 from original, but not the copy.
    if (!correct(copy5, 2 * TESTSIZE - 1, 2 * TESTSIZE, items))
        return 0;
